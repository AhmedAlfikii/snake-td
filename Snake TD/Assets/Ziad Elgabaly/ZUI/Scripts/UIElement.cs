...
namespace ZUI
{
    ...
    public class UIElement : ZUIElementBase
    {
        ...
        public class AnimationSection
        {
            ...
            public ZUIEquations.MotionType HideType = ZUIEquations.MotionType.EaseOut;
            [Tooltip("Type of showing animation.")]
            public ZUIEquations.MotionType ShowType = ZUIEquations.MotionType.EaseOut;
            ...
            public AnimationSection(bool use, Vector3 wantedVec)
            {
                ...
                HideType = ZUIEquations.MotionType.EaseOut;
                ShowType = ZUIEquations.MotionType.EaseOut;
                TypeLink = true;
                ShowAfter = -1;
                HideAfter = -1;
                ShowingDuration = -1;
        ...
        private Canvas parentCanvasScaler;
        ...
        private IEnumerator deactivateAfterEnum;
        #endregion
        ...
        public void Initialize()
        {
            ...
            parentCanvasScaler = GetComponentInParent<Canvas>();

            if (!parentCanvasScaler)
                parentCanvasScaler = GetComponent<Canvas>();

            //I added this recently to prevent errors from occuring if the object was asked to initialize while it's inactive.
        ...
        public override void ChangeVisibility(bool visible, bool trivial = false)
        {
            ...
            if (MovementSection.UseSection)
            {
                ...
                ZUIEquations.MotionType type = GetSectionType(MovementSection);
            ...
            if (RotationSection.UseSection)
            {
                ...
                ZUIEquations.MotionType type = GetSectionType(RotationSection);
            ...
            if (ScaleSection.UseSection)
            {
                ...
                ZUIEquations.MotionType type = GetSectionType(ScaleSection);
            ...
            if (OpacitySection.UseSection)
            {
                ...
                ZUIEquations.MotionType type = GetSectionType(OpacitySection);
            ...
            if (SliceSection.UseSection)
            {
                ...
                ZUIEquations.MotionType type = GetSectionType(SliceSection);
            ...
            if (DeactivateWhileInvisible)
            {
                if (!visible)
                {
                    deactivateAfterEnum = DeactivateMeAfter(hidingTime);
                    StartCoroutine(deactivateAfterEnum);
                }
                else
                {
                    if (deactivateAfterEnum != null)
                    {
                        StopCoroutine(deactivateAfterEnum);
                        deactivateAfterEnum = null;
                    }
                }
            }
        }
        ...
        public void ControlMovement(bool visible, ZUIEquations.MotionType motionType, ScreenSides side, float duration, EasingEquationsParameters easingParams, float edgeGap = 0.25f, bool instantStart = false, Vector3 customPosition = new Vector3(), bool customLocal = false)
        {
        ...
        /// <param name="duration"></param>
        public void ControlRotation(bool visible, ZUIEquations.MotionType motionType, RotationDirection direction, Vector3 euler, float duration, EasingEquationsParameters easingParams, bool instantStart = false)
        {
        ...
        /// <param name="duration"></param>
        public void ControlScale(bool visible, ZUIEquations.MotionType motionType, Vector3 scale, float duration, EasingEquationsParameters easingParams, bool instantStart = false)
        {
        ...
        /// <param name="duration"></param>
        public void ControlOpacity(bool visible, ZUIEquations.MotionType motionType, float opac, float duration, EasingEquationsParameters easingParams, bool instantStart = false)
        {
            if (!TargetFader)
                FindTargetFader();
        ...
        /// <param name="duration"></param>
        public void ControlSlice(bool visible, ZUIEquations.MotionType motionType, float fill, float duration, EasingEquationsParameters easingParams, bool instantStart = false)
        {
            if (!SliceImage)
                FindSliceImage();
        ...
        IEnumerator VectorMotion(UnityAction<Vector3> output, Vector3 start, Vector3 end, float sectionHideAfter, float sectionShowAfter, float duration, EasingEquationsParameters easingParams, ZUIEquations.MotionType motionType, bool instantStart = false)
        {
            if (!instantStart)
            {
                ...
                if (UseUnscaledTime)
                    yield return new WaitForSecondsRealtime(startAfter);
                else
        ...
        IEnumerator FloatMotion(UnityAction<float> output, float start, float end, float sectionHideAfter, float sectionShowAfter, float duration, EasingEquationsParameters easingParams, ZUIEquations.MotionType motionType, bool instantStart = false)
        {
            if (!instantStart)
            {
                float startAfter = Visible ? (sectionShowAfter < 0 ? ShowAfter : sectionShowAfter) :
        ...
        ZUIEquations.MotionType GetSectionType(AnimationSection s)
        {
